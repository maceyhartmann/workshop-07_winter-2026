...
H~0~: **pH does not predict abalone growth (change in shell surface area per day, mm^2^ day^-1^) **  

H~A~: **pH predicts abalone growth (change in shell surface area per day, mm^2^ day^-1^)**

## b. Cleaning

```{r abalone-data-cleaning}
# creating a new object called abalone clean 
...
ggplot(data = abalone_clean, # ggplot should be clean
       aes(x = mean_ph, # x - axis label
           y = change_in_area)) +
...
**Personal notes** Residuals seem homoscedastic (consistent across values) based on Residuals vs fitted and Scale-location plots. Residuals also seem normally distributed based on QQ plot. No major outliers based on Residuals vs. Leverage 
...
**std_error: standard error, precision of the predictor**
...
**This object is a ggeffects object AND a data frame**  
...
**at pH = 8 the model predicts that abalone change in shell surface area per day should be 5.36 mm^2^ d^-1^ (95% CI: [5.88, 5.64]**
...
**https://maceyhartmann.github.io/workshop-07_winter-2026/code/abalone-example.html**
